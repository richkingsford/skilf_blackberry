...
if ($raw -notmatch '<div class="home-final-actions"[\s\S]*?href="apply\.html"[\s\S]*?>\s*Begin your HighBar path[\s\S]*?href="monetize\.html"[\s\S]*?>\s*Hire or mentor an intern\s*</a>[\s\S]*?</div>') {
  throw 'Expected homepage bottom CTAs to be Begin your HighBar path and Hire or mentor an intern.'
}
